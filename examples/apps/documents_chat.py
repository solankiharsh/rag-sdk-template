...
from ragsdk.core.llms.litellm import LiteLLM  # type: ignore  # noqa: F811
...
class RAGSystemWithUI:
    ...
    def __init__(
        ...
        index_name: str = "documents-harsh",
        ...
        self._llm = LiteLLM(model_name, api_key=api_key, use_structured_output=True)
    ...
    async def _handle_message(
        ...
        prompt = RAGPrompt(
            QueryWithContext(
                query=message,
                context=[i.text_representation for i in results if i.text_representation]
            )
        )
    ...
    def prepare_layout(self) -> gr.Blocks:
        ...
        with gr.Blocks(fill_height=True, fill_width=True) as app, gr.Row():
            with gr.Column(scale=self._columns_ratios[0]):
                with gr.Group():
                    documents_picker = gr.File(
                        file_count="multiple",
                        label=self.DOCUMENT_PICKER_LABEL
                    )
                ...
                load_btn.click(
                    ...
                    outputs=loading_status_display
                )
                create_btn.click(
                    fn=self._create_database,
                    inputs=documents_picker,
                    outputs=creating_status_display
                )
