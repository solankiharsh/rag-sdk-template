...
def ragsdk_cli(
    ...
    """Common CLI arguments for all ragsdk commands."""
...
def autoregister() -> None:
    """
    Autodiscover and register all the CLI modules in the ragsdk packages.
    ...
    for module in cli_enabled_modules:
        register_func = importlib.import_module(f"ragsdk.{module.name}.cli").register
        register_func(app)
