...
from pydantic.alias_generators import to_snake  # type: ignore
from rich.console import Console  # type: ignore

from ragsdk.core.config import CoreConfig, core_config  # type: ignore
...
WithConstructionConfigT_co = TypeVar(
    "WithConstructionConfigT_co",
    bound=WithConstructionConfig,
    covariant=True
)


# Using a Protocol instead of simply typing the `cls` argument to `get_instance_or_exit`
# as `type[WithConstructionConfigT]` in order to workaround the issue of mypy not
# allowing abstract classes to be used as types: https://github.com/python/mypy/issues/4717
...
def get_instance_or_exit(
    ...
    Args:
        ...
        yaml_path_argument_name: The name of the argument
            to use in error messages for the YAML path.
        config_override: A config instance to be used
        factory_path_argument_name: The name of the argument to use in error messages for the
            factory path.
    """
    ...
    try:
        ...
        Console(
            ...
            f"""You need to provide the [b]{type_name}[/b] instance to be used.
...
- providing a Python path to a function that creates a vector store with the
  [b]{factory_path_argument_name}[/b] option
- setting the preferred {type_name} configuration in your project's [b]pyproject.toml[/b] file
  (see https://github.com/solankiharsh/rag-sdk-50-days-of-learninghow-to/project/component_preferences/
