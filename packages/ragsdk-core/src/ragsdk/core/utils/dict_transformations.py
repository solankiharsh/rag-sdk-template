...
def flatten_dict(
    ...
    Raises:
        ValueError: If the dictionary cannot be safely flattened due to the presence
            of the separator in the dict key.
    """
    items: dict[str, SimpleTypes] = {}
    for k, v in input_dict.items():
        if sep in k:
            raise ValueError(
                f"Separator '{sep}' found in key '{parent_key}'. Cannot flatten dictionary safely."
            )

        if "[" in k or "]" in k:
            raise ValueError(
                f"Key '{k}' cannot consist '[]' characters. Cannot flatten dictionary safely."
            )
...
def unflatten_dict(input_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    and bracket notation for arrays) into a nested dictionary structure.
    It uses the notation to determine
    ...
    Returns:
        dict[str, Any]: A nested dictionary structure. Lists are created only when
