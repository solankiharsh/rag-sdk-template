...
def get_ragsdk_config(current_dir: Path | None = None) -> dict[str, Any]:
    """
    Get the ragsdk configuration from the project's pyproject.toml file.

    Only configuration from the [tool.ragsdk] section is returned.
    If the project doesn't include any ragsdk configuration, an empty dictionary is returned.
    ...
    config = pyproject_data.get("tool", {}).get("ragsdk", {})
...
def get_config_instance(
    ...
    config = get_ragsdk_config(current_dir)
    if subproject:
        config = {
