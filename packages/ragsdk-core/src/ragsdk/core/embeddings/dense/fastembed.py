...
from ragsdk.core.embeddings.dense.base import DenseEmbedder, EmbedderOptionsT
...
class FastEmbedEmbedder(DenseEmbedder[FastEmbedOptions]):
    ...
    def __init__(
        ...
        model_name: str,
        use_gpu: bool = False,
        default_options: FastEmbedOptions | None = None,
    ):
    ...
    async def get_vector_size(self) -> VectorSize:
        ...
        model_info = next(
            (model for model in supported_models if model["model"] == self.model_name),
            None
        )
