...
from ragsdk.core.embeddings import DenseEmbedder, Embedder, SparseVector
from ragsdk.core.options import Options
from ragsdk.core.utils.config_handling import ConfigurableComponent, ObjectConstructionConfig
...
class VectorStoreResult(BaseModel):
    ...
    # If the results were created by combining multiple results, this field will contain
    # the subresults.
...
class VectorStoreOptions(Options):
    ...
    Attributes:
        ...
        score_threshold: The minimum similarity score for an entry to be returned.
            ...
            for more details).
        where: The filter dictionary - the keys are the field names and the values are
            the values to filter by. Not specifying the key means no filtering.
    """
...
class VectorStore(ConfigurableComponent[VectorStoreOptionsT], ABC):
    """
    A class with an implementation of Vector Store, allowing to store and retrieve vectors
    ...
    async def list(
        ...
        Args:
            where: The filter dictionary - the keys are the field names and
...
class VectorStoreWithDenseEmbedder(VectorStore[VectorStoreOptionsT]):
    ...
    def __init__(
        ...
        Args:
            ...
            embedding_type: Which part of the entry to embed, either text or image.
                The other part will be ignored.
    ...
    async def _create_embeddings(self, entries: list[VectorStoreEntry]) -> dict[UUID, list[float]]:
        ...
        if self._embedding_type == EmbeddingType.TEXT:
            ...
            embeddings = await self._embedder.embed_text(
                [e.text for e in entries if e.text is not None]
            )
            ...
            embeddings = await self._embedder.embed_image(
                [e.image_bytes for e in entries if e.image_bytes is not None]
            )
            return {e.id: v for e, v in zip(entries, embeddings, strict=True)}
        else:
...
class VectorStoreWithEmbedder(VectorStore[VectorStoreOptionsT]):
    ...
    def __init__(
        ...
        Args:
            embedder: The embedder to use for converting entries to vectors. Can be a regular
                Embedder for dense vectors or a SparseEmbedder for sparse vectors.
            embedding_type: Which part of the entry to embed, either text or image.
                The other part will be ignored.
            default_options: The default options for querying the vector store.
        """
    ...
    async def _create_embeddings(
    ...
    ) -> dict[UUID, list[float] | SparseVector]:
        """
        Create embeddings for the given entry, using the provided embedder and embedding type.
        ...
        if self._embedding_type == EmbeddingType.TEXT:
            ...
            return {
                e.id: cast(SparseVector | list[float], v)
                for e, v in zip(entries, embeddings, strict=True)
            }
        elif self._embedding_type == EmbeddingType.IMAGE:
            entries = [e for e in entries if e.image_bytes is not None]
            embeddings = await self._embedder.embed_image(
