...
from qdrant_client.local.distances import DistanceOrder, distance_to_order  # type: ignore
...
from ragsdk.core.embeddings import Embedder, SparseEmbedder, SparseVector
...
class QdrantVectorStore(VectorStoreWithEmbedder[VectorStoreOptions]):
    ...
    def __reduce__(self) -> tuple[Callable, tuple]:
        ...
        Returns:
            The tuple of function and its arguments that allows reconstruction of the
            QdrantVectorStore.
        """
    ...
    async def retrieve(
        ...
        # Ragsdk has a "larger is better" convention for all scores, so we need to reverse
        # the score if the distance method is "smaller is better".
    ...
    def _create_qdrant_filter(where: WhereQuery | None) -> Filter:
        ...
        return Filter(
            must=[
                FieldCondition(
                    key=f"metadata.{key}",
                    match=MatchValue(value=cast(str | int | bool, value))
                )
    ...
    async def list(
        ...
        with trace(where=where, index_name=self._index_name, limit=limit, offset=offset) as outputs:
            ...
            outputs.results = [
                VectorStoreEntry.model_validate(point.payload)
