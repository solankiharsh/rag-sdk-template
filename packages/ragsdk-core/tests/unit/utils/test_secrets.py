...
from ragsdk.core.utils.secrets import RAGSDK_KEY_ENV_VAR, get_secret_key  # type: ignore
...
def test_get_secret_key_from_env():
    ...
    with patch.dict(os.environ, {RAGSDK_KEY_ENV_VAR: test_key}, clear=True):
...
def test_get_secret_key_warning():
    ...
    with (
        ...
        pytest.warns(
            UserWarning,
            match=f"No secret key found in environment variable {RAGSDK_KEY_ENV_VAR}"
        ),
    ):
        get_secret_key(env_var=RAGSDK_KEY_ENV_VAR)
