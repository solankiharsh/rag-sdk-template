...
from ragsdk.core.vector_stores.base import VectorStore, VectorStoreOptions
from ragsdk.core.vector_stores.chroma import ChromaVectorStore
from ragsdk.core.vector_stores.in_memory import InMemoryVectorStore
from ragsdk.core.vector_stores.qdrant import QdrantVectorStore


def test_subclass_from_config():
    config = ObjectConstructionConfig.model_validate(
        {
            "type": "ragsdk.core.vector_stores:InMemoryVectorStore",
            "config": {
                "default_options": {
                    "k": 10,
                ...
                "embedder": {
                    "type": "ragsdk.core.embeddings:NoopEmbedder",
                },
            },
        }
    )
...
def test_subclass_from_config_chroma_client():
    config = ObjectConstructionConfig.model_validate(
        {
            "type": "ragsdk.core.vector_stores.chroma:ChromaVectorStore",
            "config": {
...
def test_subclass_from_config_qdrant_client():
    config = ObjectConstructionConfig.model_validate(
        {
            "type": "ragsdk.core.vector_stores.qdrant:QdrantVectorStore",
            "config": {
                "client": {
