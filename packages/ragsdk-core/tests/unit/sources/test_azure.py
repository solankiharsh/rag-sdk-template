...
from ragsdk.core.sources.exceptions import SourceNotFoundError
...
def test_get_blob_service_with_default_credentials():
    ...
    with (
        patch("ragsdk.core.sources.azure.DefaultAzureCredential") as mock_credential,
        patch("ragsdk.core.sources.azure.BlobServiceClient") as mock_blob_client,
        patch("azure.storage.blob.BlobServiceClient.from_connection_string") as mock_from_connection_string,
    ):
...
async def test_fetch():
    ...
    with (
        ...
        patch(
            "ragsdk.core.sources.azure.get_local_storage_dir",
