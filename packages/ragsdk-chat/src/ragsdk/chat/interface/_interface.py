...
from ragsdk.core.utils import get_secret_key  # type: ignore
...
def with_chat_metadata(
    func: Callable[
        ["ChatInterface", str, ChatFormat | None, ChatContext | None],
    ...
    """
    Decorator that adds message and conversation metadata to the chat method and
    ...
    async def wrapper(
        ...
        if not context.conversation_id and (not history or len(history) == 0):
            ...
            yield ChatResponse(
                ...
                content=context.conversation_id
            )
...
class ChatInterface(ABC):
    ...
    * State updates: Updates to the conversation state
    ...
    async def setup(self) -> None:  # noqa: B027
        ...
        This method is called after the chat interface is initialized
        and before the chat method is called.
        It is used to setup the chat interface, such as loading the model
        or initializing the vector store.
    ...
    async def chat(
        ...
        yield ChatResponse(
            ...
            content="Ragsdk cannot respond - please implement chat method!"
        )
