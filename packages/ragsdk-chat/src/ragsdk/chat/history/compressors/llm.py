...
from ragsdk.chat.history.compressors import ConversationHistoryCompressor
...
class StandaloneMessageCompressorPrompt(Prompt[LastMessageAndHistory, str]):
    ...
    system_prompt = """
    Given a new message and a history of the conversation,
    create a standalone version of the message.
    ...
    Do NOT return the history, do NOT answer the question,
    and do NOT add context irrelevant to the message.
    """
...
class StandaloneMessageCompressor(ConversationHistoryCompressor):
    ...
    def __init__(
        ...
        llm: LLM,
        history_len: int = 5,
        prompt: type[Prompt[LastMessageAndHistory, str]] | None = None,
    ):
        """
    ...
    async def compress(self, conversation: ChatFormat) -> str:
        ...
        Args:
            conversation: List of dicts with "role" and "content" keys, representing
                the chat history so far. The most recent message should be from the user.
        """
        ...
        if last_message["role"] != "user":
            raise ValueError(
                "StandaloneMessageCompressor expects the last message to be from the user."
            )

        # Only include "user" and "assistant" messages in the history
        other_messages = [
            message for message in conversation[:-1]
        ...
        history = [
            ...
            for message in other_messages[-self._history_len:]
        ]
