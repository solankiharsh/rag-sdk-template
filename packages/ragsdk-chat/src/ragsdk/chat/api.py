...
from ragsdk.chat.interface.types import ChatContext, ChatResponse, Message
...
class RagAPI:
    """
    RagAPI class for running API with Demo UI for testing purposes
    """
    ...
    def __init__(
        ...
        Args:
            ...
            cors_origins: List of allowed CORS origins.
                If None, defaults to common development origins.
    ...
    def configure_app(self) -> None:
        ...
        # Note: Assets directory is always produced by the build process,
        # but static directory is optional
    ...
    def setup_routes(self) -> None:
        ...
        async def chat_message(request: ChatMessageRequest) -> StreamingResponse:
            if not self.chat_interface:
                raise HTTPException(
                    status_code=500,
                    detail="Chat implementation is not initialized"
                )
            ...
            if "state" in request.context and "signature" in request.context:
                ...
                if not ChatInterface.verify_state(state, signature):
                    logger.warning(
                        f"Invalid state signature received for message {chat_context.message_id}"
                    )
                    raise HTTPException(
                ...
                # (it's already parsed into ChatContext)
            ...
            return StreamingResponse(
                RagAPI._chat_response_to_sse(response_generator),
    ...
    def _load_chat_interface(implementation: type[ChatInterface] | str) -> ChatInterface:
        ...
        if isinstance(implementation, str):
            ...
            logger.info(
                f"Loading chat implementation from path: {module_stringified}, "
                f"class: {object_stringified}"
            )
