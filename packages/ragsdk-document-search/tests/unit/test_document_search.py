...
CONFIG = {
    "vector_store": {
        "type": "ragsdk.core.vector_stores.in_memory:InMemoryVectorStore",
        "config": {
...
async def test_document_search_ingest_from_huggingface_uri_basic():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with (
            mock.patch("ragsdk.core.sources.hf.load_dataset", return_value=dataset),
            mock.patch("ragsdk.core.sources.base.get_local_storage_dir", return_value=storage_dir),
        ):
            document_search: DocumentSearch = DocumentSearch(
