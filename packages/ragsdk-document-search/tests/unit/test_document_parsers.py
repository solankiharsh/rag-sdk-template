...
@pytest.mark.parametrize(
    ...
    [
        ("ragsdk.document_search.ingestion.parsers.base:TextDocumentParser", TextDocumentParser),
        ("ragsdk.document_search.ingestion.parsers.base:ImageDocumentParser", ImageDocumentParser),
        (
            "ragsdk.document_search.ingestion.parsers.unstructured:UnstructuredDocumentParser",
...
def test_parser_subclass_from_config(
    parser_type: str, expected_parser: type[DocumentParser]
) -> None:
...
async def test_parser_call_fail(parser_type: type[DocumentParser]) -> None:
    ...
    assert exc.value.message == (
        f"Document type {DocumentType.PDF.value} is not supported by the "
        f"{parser_type.__name__}"
    )
