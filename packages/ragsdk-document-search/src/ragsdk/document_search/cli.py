...
def register(app: typer.Typer) -> None:
    ...
    app.add_typer(
        ...
        help="Commands for interacting with the document search"
    )
...
def common_args(
    ...
    yaml_path: Annotated[
        Path | None,
        typer.Option(
            help="Path to a YAML configuration file for the document search",
            exists=True,
            resolve_path=True,
        ),
    ] = None,
...
def search(
    ...
    async def run() -> None:
        ...
        options: DocumentSearchOptions = DocumentSearchOptions(
            vector_store_options=VectorStoreOptions(k=k)
        )
