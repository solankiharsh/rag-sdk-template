...
from ragsdk import document_search
from ragsdk.core.audit.traces import trace, traceable  # type: ignore
from ragsdk.core.config import CoreConfig  # type: ignore
from ragsdk.core.options import Options  # type: ignore
from ragsdk.core.sources.base import Source, SourceResolver  # type: ignore
from ragsdk.core.types import NOT_GIVEN, NotGiven  # type: ignore
from ragsdk.core.utils._pyproject import get_config_from_yaml  # type: ignore
from ragsdk.core.utils.config_handling import (  # type: ignore
    ConfigurableComponent,
    NoPreferredConfigError,
    ObjectConstructionConfig,
)
from ragsdk.core.vector_stores.base import VectorStore, VectorStoreOptionsT  # type: ignore
...
from ragsdk.document_search.ingestion.strategies.base import (
    IngestExecutionError,
    IngestExecutionResult,
    IngestStrategy,
)
from ragsdk.document_search.ingestion.strategies.sequential import SequentialIngestStrategy
from ragsdk.document_search.retrieval.rephrasers.base import QueryRephraser, QueryRephraserOptionsT
from ragsdk.document_search.retrieval.rephrasers.noop import NoopQueryRephraser
from ragsdk.document_search.retrieval.rerankers.base import Reranker, RerankerOptionsT
from ragsdk.document_search.retrieval.rerankers.noop import NoopReranker

# Define specific type variables for DocumentSearchOptions
QRO = TypeVar('QRO')
VSO = TypeVar('VSO')
RO = TypeVar('RO')

class DocumentSearchOptions(
    Options, Generic[QRO, VSO, RO]
):
    """
    ...
    """
    query_rephraser_options: QRO | None | NotGiven = NOT_GIVEN
    vector_store_options: VSO | None | NotGiven = NOT_GIVEN
    reranker_options: RO | None | NotGiven = NOT_GIVEN
...
class DocumentSearchConfig(BaseModel):
    ...
    ingest_strategy: ObjectConstructionConfig = (
        ObjectConstructionConfig(type="SequentialIngestStrategy")
    )
...
class DocumentSearch(
    ConfigurableComponent[
        DocumentSearchOptions[QueryRephraserOptionsT, VectorStoreOptionsT, RerankerOptionsT]
    ]
):
    """
    Main entrypoint to the document search functionality.
    It provides methods for document retrieval and ingestion.
    ...
    Ingestion:
        ...
        2. Uses DocumentParserRouter to route the document to the appropriate DocumentParser
           to parse the content.
        3. Uses ElementEnricherRouter to redirect the element to the appropriate ElementEnricher
    ...
    def from_config(cls, config: dict) -> Self:
        ...
        Raises:
            ...
            InvalidConfigError: If one of the specified classes can't be found or is not
                the correct type.
        """
    ...
    def preferred_subclass(
        ...
        Args:
            ...
            yaml_path_override: A string representing the path to the YAML file containing
                the Ragstack instance configuration. Looks for the configuration under
                the key "document_search", and if not found, instantiates the class with the
                preferred configuration for each component.
        ...
        if yaml_path_override:
            ...
            if type_config := preferences.get(cls.configuration_key):
                return cls.subclass_from_config(
                    ObjectConstructionConfig.model_validate(type_config)
                )
        ...
        if config.component_preference_config_path is not None:
            ...
            if preferred_config := config.preferred_instances_config.get(cls.configuration_key):
                return cls.subclass_from_config(
                    ObjectConstructionConfig.model_validate(preferred_config)
                )

            # Instantiate the class with the preferred configuration for each component
            return cls.from_config(config.preferred_instances_config)

        raise NoPreferredConfigError(
            f"Could not find preferred factory or configuration for {cls.configuration_key}"
        )
    ...
    async def ingest(
        ...
        Args:
            documents: A string representing a source-specific URI (e.g., "gcs://bucket/*")
                       or an iterable of
                       `Document`, `DocumentMeta`, or `Source` objects.
                       Examples of URI formats include:
                       - "file:///path/to/files/*.txt"
                       - "gcs://bucket/folder/*"
                       - "huggingface://dataset/split/row"
            fail_on_error: If True, raises IngestExecutionError when any errors are
        ...
        Raises:
            IngestExecutionError: If fail_on_error is True and any errors are
                encountered during ingestion.
        """
        resolved_documents = (
            await SourceResolver.resolve(documents) if isinstance(documents, str) else documents
        )
